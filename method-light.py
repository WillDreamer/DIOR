...
def early(loss_ ,net, data, label, optimizer, criterion, nonzero_ratio, clip):
    ...
    for name, param in net.named_parameters():
        if name in ['model.classifier.3.weight','hash_layer.1.weight']:
...
def train(train_dataloader,
    ...
    criterion_new = warmup(multiclass=multi_labels)
    ...
    for epoch in range(max_iter):
        ...
        for iters, (data, data_aug, target, index) in enumerate(train_dataloader):
            ...
            if epoch < warm_up:
                ...
                endss = time.time() - startss
                time_forss += endss
                
                loss = ploss + me_max
                loss, timepp = early(loss, model, clean_data, target_clean, optimizer, criterion, clip, clip)
                time_forpp += timepp
        print('Time for Sample Selection per Epoch {:.4f}'.format(time_forss))
        print('Time for Parameter Partition per Epoch {:.4f}'.format(time_forpp))
        end = time.time() - start
